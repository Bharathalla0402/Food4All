...
- (void)createSearchBar {
        if (self.tableView && !self.tableView.tableHeaderView) {
            ...
            UIButton *Backbutt=[[UIButton alloc] initWithFrame:CGRectMake(20, 5, 30, 30)];
            [Backbutt setImage:[UIImage imageNamed:@"left-arrow-angle.png"] forState:UIControlStateNormal];
           ...
           // [Backbutt addTarget:self action:@selector(BackbuttClicked5:) forControlEvents:UIControlEventTouchUpInside];
            Backbutt.backgroundColor=[UIColor clearColor];
            [customview addSubview:Backbutt];
            
            UISearchBar * theSearchBar = [[UISearchBar alloc] initWithFrame:CGRectMake(0,0,self.view.frame.size.width-30,40)]; // frame has no effect.
            theSearchBar.delegate = self;
            
            theSearchBar.showsCancelButton = NO;
            [customview addSubview:theSearchBar];
...
- (void)viewDidLoad
{
    ...
    self.navigationController.navigationBar.barTintColor = [UIColor colorWithRed:141.0/255.0f green:197.0/255.0f blue:62.0/255.0f alpha:0.9];
    self.navigationController.navigationBar.tintColor = [UIColor whiteColor];
    [[self navigationController] setNavigationBarHidden:NO animated:YES];
    self.title=@"Countrycodes";
    
    [self createSearchBar];
    
    
    NSLocale *locale = [NSLocale currentLocale];
    NSArray *countryCodes = countryCodes ? countryCodes : [NSLocale ISOCountryCodes];
